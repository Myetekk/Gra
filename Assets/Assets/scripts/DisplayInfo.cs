...
public class DisplayInfo : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public void OnPointerEnter(PointerEventData eventData) //wykonuje siê gdy nakieruje siê kursor na token savea
    {
        if (CurrentInfoCard == null)
        {
            ...
            TextMeshProUGUI t = CurrentInfoCard.transform.GetChild(0).GetComponent<TMPro.TextMeshProUGUI>(); //uzywam textmeshpro - text zamiast legacy text, dlatego tak dziwnie to jest
            t.text = "CHUJ"; //zast¹piæ wczytywanymi informacjami o zapisie gry
        }
    }
